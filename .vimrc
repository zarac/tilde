...
"" Functions
  ...
  function! SimpleFoldText() "
      return getline(v:foldstart).' --- ( '.(1 + v:foldend - v:foldstart).' lines ) '
  endfunction
...
"" :autocmd
  ...
  "au BufRead,BufNewFile *.py set ai
...
"" Testing
  "" Sav (Save As) command, to work with :windo, :bufdo and :argdo
  "" e.g. :argdo Sav /tmp/backup
  command! -nargs=1 -complete=dir Sav echo :sav join(['<args>', expand('%:t')], '/')
...
"" Notes / Tips / Tricks!
  ...
  " Insert Mode mappings
    ...
    " CTRL-[
    "    Exit insert mode Esc alternative
    " CTRL-C
    "    Exit insert mode Esc alternative
    " CTRL-H
